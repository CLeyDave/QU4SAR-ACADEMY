...
function renderSchedule(){
  ...
  var weekFilter='<div style="margin-bottom:14px;display:flex;gap:12px;align-items:center;flex-wrap:wrap">'+
    ...
    '<select class="input-field" id="sf_weekFilter" onchange="renderSchedule()" style="width:auto;min-width:200px;padding:6px 10px;font-size:13px">'+weekOpts+'</select></div>';
...
function delSched(id){if(!confirmDel())return;var item=DATA.schedule.find(function(s){return s.id===id});if(item) saveHistory(item,'deleted');DATA.schedule=DATA.schedule.filter(function(s){return s.id!==id});saveData(DATA);renderSchedule();updateCounts();toast('Horario eliminado');}
