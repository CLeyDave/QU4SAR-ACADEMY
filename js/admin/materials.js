...
function renderSection_materials(){
  ...
  document.getElementById('adminContent').innerHTML=fh+adminTable(items,['Título','Tipo','Coach','Grupo','Archivos'],function(m){
    return '<td>'+esc(m.title)+'</td><td><span class="badge badge-purple">'+esc(m.type)+'</span></td><td>'+esc(m.coach||'—')+'</td><td>'+groupName(m.group_id)+'</td><td>'+(m.image_url?'<a href="'+esc(m.image_url)+'" target="_blank" title="Ver imagen">'+ic('image',14)+'</a> ':'')+(m.attachment_url?'<a href="'+esc(m.attachment_url)+'" target="_blank" title="Descargar '+esc(m.attachment_name||'archivo')+'">'+ic('paperclip',14)+'</a>':'')+'</td><td><div class=" admin-actions"><button onclick="materialForm(\''+m.id+'\')">'+ic('pencil',14)+'</button><button class="del" onclick="delMaterial(\''+m.id+'\')">'+ic('trash-2',14)+'</button></div></td>'},'No hay materiales',btn,'delMaterial');
}
...
function materialForm(id){
  ...
  openModal('<button class="modal-close" onclick="closeModal()">'+ic('x',16)+'</button><h3>'+(item?'Editar':'Nuevo')+' Material</h3>'+
    '<div class="field"><label for="mf_schedule_id">Clase</label><select class="input-field" id="mf_schedule_id">'+(classes.length?classes.map(function(c){var days=['Lun','Mar','Mié','Jue','Vie','Sáb','Dom'];return'<option value="'+c.id+'" '+(c.id===f.schedule_id?'selected':'')+'>'+esc(c.title)+' ('+days[c.day]+')</option>'}).join(''):'<option value="">Sin clases de academia</option>')+'</select></div>'+
    '<div class="field"><label for="mf_title">Título</label><input class="input-field" id="mf_title" value="'+esc(f.title)+'"></div>'+
    '<div class="field"><label for="mf_description">Descripción</label><textarea class="input-field" id="mf_description" rows="3">'+esc(f.description||'')+'</textarea></div>'+
    '<div class="field"><label for="mf_url">URL</label><input class="input-field" id="mf_url" value="'+esc(f.url)+'"></div>'+
    '<div class="field"><label for="mf_type">Tipo</label><select class="input-field" id="mf_type"><option value="video" '+(f.type==='video'?'selected':'')+'>Video</option><option value="guide" '+(f.type==='guide'?'selected':'')+'>Guía</option><option value="pdf" '+(f.type==='pdf'?'selected':'')+'>PDF</option><option value="other" '+(f.type==='other'?'selected':'')+'>Otro</option></select></div>'+
    '<div class="field" style="display:none"><label for="mf_group">Grupo</label><select class="input-field" id="mf_group" onchange="reloadCoachDropdown(\'mf_coach\',this.value)"><option value="">General (Ambos)</option>'+(DATA.groups||[]).map(function(g){return'<option value="'+g.id+'"'+(f.group_id===g.id?' selected':'')+'>'+esc(g.name)+'</option>'}).join('')+'</select></div>'+
    '<div class="field"><label for="mf_coach">Coach asignado</label><select class="input-field" id="mf_coach" onchange="setGroupFromCoach(\'mf_group\',\'mf_coach\')"><option value="">Sin coach</option>'+coachOptions(f.coach||'',f.group_id)+'</select></div>'+
    '<hr>'+
