...
function renderSection_curriculum(){
  ...
  document.getElementById('adminContent').innerHTML=fh+adminTable(items,['Título','Semana','Temas','Coach','Grupo','Archivos'],function(c){
    ...
    return '<td>'+esc(c.title)+'</td><td>Semana '+c.week+'</td><td><div class="cell-preview" onclick="viewCurriculum(\''+c.id+'\')" title="Ver todos los temas">'+topics+'</div></td><td>'+esc(c.coach||'—')+'</td><td>'+groupName(c.group_id)+'</td><td>'+(c.image_url?'<a href="'+esc(c.image_url)+'" target="_blank" title="Ver imagen">'+ic('image',14)+'</a> ':'')+(c.attachment_url?'<a href="'+esc(c.attachment_url)+'" target="_blank" title="Descargar '+esc(c.attachment_name||'archivo')+'">'+ic('paperclip',14)+'</a>':'')+'</td><td><div class=" admin-actions"><button onclick="curriculumForm(\''+c.id+'\')">'+ic('pencil',14)+'</button><button class="del" onclick="delCurriculum(\''+c.id+'\')">'+ic('trash-2',14)+'</button></div></td>'},'No hay plan de estudio',btn,'delCurriculum');
}
...
function curriculumForm(id){
  ...
  openModal('<button class="modal-close" onclick="closeModal()">'+ic('x',16)+'</button><h3>'+(item?'Editar':'Nuevo')+' Tema</h3>'+
    '<div class="field"><label for="cf_title">Título</label><input class="input-field" id="cf_title" value="'+esc(f.title)+'"></div>'+
    ...
    '<div class="field"><label for="cf_week">Semana</label><input type="number" class="input-field" id="cf_week" value="'+f.week+'"></div>'+
    '<div class="field"><label for="cf_topics">Temas (uno por línea)</label><textarea class="input-field" id="cf_topics" rows="4">'+esc((f.topics||[]).join('\n'))+'</textarea></div>'+
    '<div class="field"><label for="cf_color">Color</label><input type="text" class="input-field" id="cf_color" value="'+esc(f.color)+'"></div>'+
    '<div class="field" style="display:none"><label for="cf_group">Grupo</label><select class="input-field" id="cf_group" onchange="reloadCoachDropdown(\'cf_coach\',this.value)"><option value="">General (Ambos)</option>'+(DATA.groups||[]).map(function(g){return'<option value="'+g.id+'"'+(f.group_id===g.id?' selected':'')+'>'+esc(g.name)+'</option>'}).join('')+'</select></div>'+
    '<div class="field"><label for="cf_coach">Coach asignado</label><select class="input-field" id="cf_coach" onchange="setGroupFromCoach(\'cf_group\',\'cf_coach\')"><option value="">Sin coach</option>'+coachOptions(f.coach||'',f.group_id)+'</select></div>'+
    '<hr>'+
