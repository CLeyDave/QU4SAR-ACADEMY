...
function renderSection_tasks(){
  ...
  if(Object.keys(grouped).length){h+='<div class="glass-card" style="padding:20px;margin-top:20px"><h3 style="font-family:var(--font-display);font-size:15px;margin-bottom:14px">'+ic('check-circle',16)+' Completaciones por tarea</h3>';
    Object.keys(grouped).forEach(function(tid){
      ...
      h+='<div class="" style="margin-bottom:16px;padding:16px;background:rgba(255,255,255,0.02);border-radius:8px"><strong style="font-size:14px">'+esc(task?task.title:'(eliminada)')+'</strong><div style="display:grid;gap:8px;margin-top:12px">';
...
async function showTaskCompletions(taskId){
  ...
  if(gid)members=members.filter(function(m){return m.group_id&&m.group_id.trim()===gid});
  ...
  if(!members.length){
    ...
    members.forEach(function(m){
      ...
      html+='<div class="" style="display:flex;justify-content:space-between;align-items:center;padding:8px 14px;background:rgba(255,255,255,0.02);border-radius:8px">'+
